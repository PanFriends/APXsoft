proc sfout {{predelta 0.1} {uponly 0} {doonly 0}} {

    #sfout

    #This is essentially identical to fscout.tcl. The output file is different.

    #Assumes a constant between first zpo and zphabs
    ...
    #Sf output
    set outtex [ open "rsf_$suffix.tex" "w" ]
    set outtxt [ open "sf_$suffix.txt" "w" ]
    ...
    set npar [expr $par1+3]

    #Exists?
    if {$npar == 0} {
	puts "nonexistent"
	exec echo "0"  > sf_$suffix.txt
        exec echo "&$\\ldots$" > rsf_$suffix.tex 
	...
	#    exec /bin/sh -c "echo $parval 0 0 0 0 > sf_$suffix.txt"
	#} else {
	    exec echo sfout $npar $predelta $uponly $doonly > "sfout_Call_$type.txt"
	    errout $npar $predelta $uponly $doonly
	    exec cp errout.txt sf_$suffix.txt
	    exec gawk {{printf"& \\aer{%.3f}{+%.3f}{-%.3f}\n", $1, $4-$1, $1-$2}} sf_$suffix.txt > rsf_$suffix.tex
    ...
    puts " "
    exec cat sf_$suffix.txt rsf_$suffix.tex
