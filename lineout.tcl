proc lineout {par {predelta 0.1} {uponly 0} {doonly 0}} {
...
exec /bin/sh -c "echo $parval f > Line{$par}_$type.txt"
}
...
tclout param $par
scan $xspec_tclout "%f" parval
exec /bin/sh -c "echo $parval 0 0 0 0 > Line{$par}_$type.txt"
} else {

exec echo lineout $par $predelta $uponly $doonly > lineout_Call_{$par}_$type.txt
errout $par $predelta $uponly $doonly
exec cp errout.txt Line{$par}_$type.txt
}
}

puts " "
exec more Line{$par}_$type.txt
}
