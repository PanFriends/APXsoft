proc snh {{nstepsj 7} {nstepsi 7} {x1 0} {x2 10} {y1 0} {y2 0.05} {linx 0} {liny 0} } {
...
# snh <specific range values>  <linear step x =0> <linear step y =0> 
...
#i for steps gsmooth; j for steps NH
    set i $nstepsi 
    set j $nstepsj
...
#    echo sigNH $fs $fz > sigNH_$type"_"Call.txt
    ...
    puts $out [format "signh\_spec %i %.4e %.4e %.4e %.4e" $nstepsj $nstepsi $x1 $x2 $y1 $y2] 
    close $out
