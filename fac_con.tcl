proc fac_con {{nsteps 15} {x1 -888} {x2 -888} {y1 -888} {y2 -888}} {
    ...
    puts "............................."
    puts "y-options: As NHs NHsph sig NHz"
    puts "x-options: Cpintoxis NHs Γs z f"
    puts "............................."
	...
	#Find NHsph, Γsph, zsph
    if { [string first trans $xspec_tclout] != -1 } {    
    scan $xspec_tclout "%s %i %i" name par npars
    set par_NHsph $par
    puts " NHsph par is $par_NHsph"
    set par_gammasph [expr $par+1]
    puts " Γsph is $par_gammasph"
    set par_zsph [expr $par+4]
    puts " zsph is $par_zsph"
	
    #Read value
    tclout param $par_NHsph
    scan $xspec_tclout "%f" NHsph
    
    tclout param $par_zsph
    scan $xspec_tclout "%f" zsph

    tclout param $par_gammasph
    scan $xspec_tclout "%f" gammasph
    }
...
#    }
    ...
    if {[file exists "fac_con_XIS_file.txt"]==1} {
	set filexis [exec more fac_con_XIS_file.txt]
    } else {
	puts "NHS XIS-only error parameters filename?"
	set filexis [gets stdin]
	if {[regexp {[a-z]} $filexis]} {
	    exec echo $filexis > fac_con_XIS_file.txt
	}
...
#	set xishigh [expr $xis4/1e2]
	set xislow [expr $xis2]
	set xishigh [expr $xis4]
#	puts "XISLOW $xislow"
    }
#    #Unless it's not MYTORUS
...
##	puts "XISLOW $xislow"
    ...
    if {[file exists "XIS-lines.txt"]==1} {
	scan [exec more XIS-lines.txt] "%i" plotlines
    }
    if {[info exists plotlines]==1} {
	if {$plotlines == 1} {
	    set toplotlines y
	} else {
	    set toplotlines n
	}
	puts "Lines from XIS-only fit?($toplotlines; <RET> or n)"
    } else {
	puts "Lines from XIS-only fit?(<RET>=y or n)"
    }
   
    set ans [gets stdin]
    if {[info exists plotlines]==0} {
	set plotlines 0
    }
    #If return hit, plot the lines
    if {[regexp {[a-zA-z]} $ans]==0} {
	set plotlines 1
    }
    #If n, don't plot
    if {[string equal "n" $ans]==1} {
	set plotlines 0
    }
    #If y, plot
    if {[string equal "y" $ans]==1} {
	set plotlines 1
    }
    exec echo $plotlines > XIS-lines.txt
    ...
    if { [string first PhoIndex $yname] != -1 } {  
	setplot command lab y \\gG\\fr\\dZ\\u 
    }
